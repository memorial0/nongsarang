/**
 * 농사랑 AI 분석 엔진 (Prototype v2.0)
 * 
 * [공지] 본 로직은 발표용 MVP를 위한 rule-based soft-sensing 프로토타입입니다.
 * 실제 서비스에서는 농촌진흥청 농사로, 토양검정 DB, 작물별 표준시비량 데이터와 
 * 현장 실증 데이터를 기반으로 고도화 예정입니다.
 */
...
// --- Logic 1: AI Calibration (Soft-Sensing) ---
export const calibrateSensorData = (
  rawN: number, rawP: number, rawK: number, 
  data: { ec: number; temp: number; moisture: number; rainfall: number; }
): CalibrationResult => {
  // 1. moistureFactor
  ...
  const calN = Number((rawN * mF * eF * tF * rF).toFixed(1));
  const calP = Number((rawP * mF * tF).toFixed(1));
  const calK = Number((rawK * mF * eF * rF).toFixed(1));
  ...
  // 극단적 수치 감점
  if (rawN > 400 || rawN < 30 || rawK > 450 || rawK < 50) conf -= 10;
  ...
  return {
    ...
    calibrationMessage: conf >= 80 ? "최적 환경 내 정밀 보정 완료" : "환경 변수 간섭에 따른 자동 보정 적용"
  };
};

// --- Logic 2: Anomaly Detection ---
export const analyzeAnomalies = (data: { n: number; ec: number; moisture: number; rainfall: number; temp: number }): Anomaly[] => {
  ...
  if (data.temp < 10 || data.temp > 35) anomalies.push({ type: 'warning', message: '온도 불안정으로 인한 생육 저하 가능성.' });
  return anomalies;
};
...
export const generatePrescription = (
  ...
  const check = (val: number, range: any, name: string, fert: string) => {
    if (val < range.min) {
      ...
      strategies.push(`${name === '칼륨(K)' ? fert + '계 비료 분할 시비' : fert + '계 비료 보충'}을 권장합니다.`);
  ...
  if (summaryParts.length === 0) {
    summaryParts.push("영양 균형 안정");
    strategies.push("현재 상태를 유지하며 정기 모니터링하십시오.");
  }

  if (env.ec >= 2.5) strategies = ["EC가 높으므로 시비량을 줄이고 관수 또는 토양 세척을 권장합니다."];
  if (env.rainfall >= 20) strategies.push("강우 중이므로 시비보다 강우 후 재진단을 권장합니다.");
  if (env.moisture < 20) strategies.push("토양이 건조하므로 관수 후 재측정을 권장합니다.");
  else if (env.moisture > 70) strategies.push("과습 상태이므로 배수 관리 후 재진단을 권장합니다.");

  return {
    summary: `현재 ${crop} 재배 기준, ${summaryParts.join(', ')} 상태입니다.`,
    strategy: strategies.join(' '),
    precautions: precautions.length > 0 ? precautions : ["표준 생육 가이드를 준수하십시오."],
